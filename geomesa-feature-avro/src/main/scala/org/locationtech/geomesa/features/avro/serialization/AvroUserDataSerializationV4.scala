...
object AvroUserDataSerializationV4 extends LazyLogging {
  ...
  def serialize(out: Encoder, map: java.util.Map[_ <: AnyRef, _ <: AnyRef]): Unit = {
    ...
    val filtered = map.asScala.filter { case (key, value) =>
      if (canSerialize(key)) {
        ...
        val s = s"Can't serialize Map entry ($key,$value) - it will be skipped."
        logger.warn(s)
        false
      }
    }
  ...
  private def deserializeWithSize(in: Decoder, size: Int, map: java.util.Map[AnyRef, AnyRef]): Unit = {
    ...
    while (remaining > 0) {
      ...
      val key = keyClass match {
        case NullMarkerString =>
          null
        case "org.geotools.factory.Hints$Key" | "org.geotools.util.factory.Hints$Key" =>
          HintKeySerialization.idToKey(in.readString())
        case _ =>
          read(in, Class.forName(keyClass))
      }
